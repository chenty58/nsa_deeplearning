...
n_epochs = 1000
batch_size = 100000
gpu_option = tf.GPUOptions(per_process_gpu_memory_fraction=0.08)
...
def convertOneHot(data,num):
    ...
    ss = tf.Session(config=tf.ConfigProto(gpu_options=gpu_option))
    array = ss.run(tensor)
    return array
...
feature = pd.read_csv('kddcup.data.csv',delimiter=',', \
                usecols=(feature_col),dtype=str,encoding="utf-8").values
...
target=pd.read_csv('kddcup.data.csv',delimiter=',',usecols=([41]),\
                                      dtype=str,encoding="utf-8").values
...
def inference_deep_layers(x_tf, A, B):
    with tf.variable_scope("hidden_1"):
        hidden_1 = layer(x_tf, [A, 40],[40])
    with tf.variable_scope("hidden_2"):
        hidden_2 = layer(hidden_1, [40, 40],[40])
    with tf.variable_scope("hidden_3"):
        hidden_3 = layer(hidden_2, [40, 40],[40])
    with tf.variable_scope("hidden_4"):
        hidden_4 = layer(hidden_3, [40, 20],[20])
    with tf.variable_scope("output"):
        output = layer(hidden_4, [20, B], [B])
    return output
...
###########################################################
## try AdamOptimizer intead of GradientDescent
