...
feature = pd.read_csv('kddcup.data.csv',delimiter=',',usecols=(featuer_col),dtype=str,encoding="utf-8").values
for c in range(1,4):
    feature[:,c]=LabelEncoder().fit_transform(feature[:,c])
feature=feature.astype(np.float)
target=pd.read_csv('kddcup.data.csv',delimiter=',',usecols=([41]),dtype=str,encoding="utf-8").values
...
x_train, x_test, y_train_onehot, y_test_onehot = train_test_split(feature_std, target_onehot, test_size=0.25, random_state=0)
A=x_train.shape[1]
